...
---------------------------------------------------------------------------------------------------------
CREATE TABLE [dbo].[ТехногенныйОбъект](
	[IdТехногенногоОбъекта] [int] NOT NULL,
	[КодТипаТехногенногоОбъекта] [int] NOT NULL,
	[КодТипаНазначенияЗемель] [int] NOT NULL,
	[НаименованиеТехногенногоОбъекта] [nvarchar](max) NOT NULL,
	[АдресТехногенногоОбъекта] [nvarchar](max) NULL,
	[ШиротаГрад] [float] NOT NULL,
	[ДолготаГрад] [float] NOT NULL,
	[ТипГрунта] [int] NOT NULL,
	[ГлубинаГрунтовыхВод] [float] NULL,
	[ВысотаУровнемМоря] [float] NULL,
 CONSTRAINT [PK_ТехногенныйОбъект] PRIMARY KEY CLUSTERED 
(
	[IdТехногенногоОбъекта] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
...
ALTER TABLE [dbo].[ТехногенныйОбъект]  WITH CHECK ADD  CONSTRAINT [FK_ТехногенныйОбъект_ТипГрунта] FOREIGN KEY([ТипГрунта])
REFERENCES [dbo].[ТипГрунта] ([КодТипаГрунта])
GO

ALTER TABLE [dbo].[ТехногенныйОбъект] CHECK CONSTRAINT [FK_ТехногенныйОбъект_ТипГрунта]
GO
